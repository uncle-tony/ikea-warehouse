...
from model.v1.product import ProductV1
...
class ClientControllerV1(Resource):
    def post(self):
        def _rollback(start, product_list):
            start-=1
            while start >= 0:
                article_roll = ArticleV1.query.filter_by(art_id=product_list.articles[start]['art_id']).first()
                if article_roll:
                    try:
        ...
        if params['action'] and params['action'] in ALLOWED_ACTIONS:
            product = ProductV1.query.filter_by(name=params['product']).first()
            if product:
                try:
                    update_counter = 0
                    for idx, article in enumerate(product.articles):
                        article_ref = ArticleV1.query.filter_by(art_id=article['art_id']).first()
                        if article_ref:
